...
class Order
{
    ...
    function update($POST, $order_detail)
    {
        $db = new Database();
        $data = array();

        //update quantity (số lượng hàng đã giao sẽ đc trừ vào slg kho)
        foreach($order_detail as $item){
            $query = "UPDATE product SET quantity = quantity - $item->qty WHERE id = $item->product_id;";
            $db->write($query);
        }
